WITH sumaKlient AS (
  SELECT
    SUM(offer.offer_price) AS klient
  from
    sale
    INNER JOIN offer_customer ON sale.offer_customer_id = offer_customer.id
    INNER JOIN offer ON offer_customer.offer_id = offer.id
    INNER JOIN customer ON offer_customer.customer_id = customer.id
  WHERE
    customer.nip = 1000000000
),
sumaKlienci AS (
  SELECT
    SUM(offer.offer_price)
  from
    sale
    INNER JOIN offer_customer ON sale.offer_customer_id = offer_customer.id
    INNER JOIN offer ON offer_customer.offer_id = offer.id
...
SELECT
  SUM(klienci.wiecej) AS WydaliWiecej,
  SUM(klienci.mniej) AS WydaliMniej
FROM
  (
    SELECT
      CASE WHEN (
